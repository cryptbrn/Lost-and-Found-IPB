...
class CreatePostActivity : AppCompatActivity() {
    ...
    private fun init(){
        ...
        typePost = findViewById(R.id.create_post_type)
        itemCategory = findViewById(R.id.create_post_category)
    }
    private fun onClick() {
        create_post_btn_post.setOnClickListener { checkPost() }
        create_post_picture_layout.setOnClickListener { chooseMethod() }
    }
    ...
    private fun checkPost() {
        create_post_title.error = null
        create_post_desc.error = null
        create_post_name.error = null
        create_post_location.error = null

        var cancel = false
        var focusView: View? = null

        type = typePost.selectedItem.toString()
        title = create_post_title.text.toString()
        description = create_post_desc.text.toString()
        name = create_post_name.text.toString()
        category = itemCategory.selectedItem.toString()
        location = create_post_location.text.toString()
        ...
        if(type=="-- Select Type --"){
            ...
            cancel = true
            focusView = create_post_type
        }
        if (TextUtils.isEmpty(title)) {
            create_post_title.error = getString(R.string.error_empty)
            focusView = create_post_title
            cancel = true
        }
        if (TextUtils.isEmpty(description)) {
            create_post_desc.error = getString(R.string.error_empty)
            focusView = create_post_desc
            cancel = true
        }
        if (TextUtils.isEmpty(name)) {
            create_post_name.error = getString(R.string.error_empty)
            focusView = create_post_name
            cancel = true
        }
        if(category=="-- Select Category --"){
            toast("Please select Item Category first")
            cancel = true
            focusView = create_post_category
        }
        if (TextUtils.isEmpty(location)) {
            create_post_location.error = getString(R.string.error_empty)
            focusView = create_post_location
            cancel = true
        }
        if(pictureReq.body().contentLength()<1){
            toast("Please insert Item Image first")
            focusView = create_post_picture_layout
            cancel = true
        }
    ...
    private fun showProgress(show: Boolean){
        create_post_progress.visibility = if(show) View.VISIBLE else View.GONE
        disableTouch(show)
    }
