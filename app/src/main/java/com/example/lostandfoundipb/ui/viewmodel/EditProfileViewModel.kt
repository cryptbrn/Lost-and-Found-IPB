...
class EditProfileViewModel : ViewModel() {
    ...
    fun editProfile(api: ApiService, update: Map<String, @JvmSuppressWildcards RequestBody>) = viewModelScope.launch {
        val response = api.editProfile(update)
        editDetailResult.postValue(handleEditDetailResponse(response)!!)
    }

    fun editProfile(api: ApiService, picture: MultipartBody.Part, update: Map<String, @JvmSuppressWildcards RequestBody>) = viewModelScope.launch {
        val response = api.editProfile(picture,update)
        editDetailResult.postValue(handleEditDetailResponse(response)!!)
    }
